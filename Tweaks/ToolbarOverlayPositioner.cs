...
namespace ExtendedToolbar.Tweaks
{
    ...
    {
        private readonly ToolbarLayoutManager manager;
        ...
        public ToolbarOverlayPositioner(ToolbarLayoutManager manager, IOsuCcPluginHost host)
        {
        ...
        private void resolveOverlays()
        {
            try
            {
                if (loginOverlay == null && manager.AllBlocks.TryGetValue("user_profile", out var userBlock))
                {
                    ...
                    {
                        loggedLogin = true;
                        ExtendedToolbarLog.Info($"ToolbarOverlayPositioner: LoginOverlay extracted from ToolbarUserButton ({loginOverlay.GetType().FullName}, Hash={loginOverlay.GetHashCode()})");
                    }
                }
                ...
                if (musicOverlay == null && manager.AllBlocks.TryGetValue("music", out var musicBlock))
                {
                    ...
                    {
                        loggedMusic = true;
                        ExtendedToolbarLog.Info($"ToolbarOverlayPositioner: MusicOverlay extracted from ToolbarMusicButton ({musicOverlay.GetType().FullName}, Hash={musicOverlay.GetHashCode()})");
                    }
                }
                ...
                if (host.Game is Drawable gameRoot)
                {
                    if (loginOverlay == null)
                    {
                        ...
                        {
                            loggedLogin = true;
                            ExtendedToolbarLog.Info($"ToolbarOverlayPositioner: LoginOverlay found in game tree ({loginOverlay.GetType().FullName}, Hash={loginOverlay.GetHashCode()})");
                        }
                    }
                    ...
                    if (musicOverlay == null)
                    {
                        ...
                        {
                            loggedMusic = true;
                            ExtendedToolbarLog.Info($"ToolbarOverlayPositioner: MusicOverlay found in game tree ({musicOverlay.GetType().FullName}, Hash={musicOverlay.GetHashCode()})");
                        }
                    }
                }
            }
            ...
            {
                ExtendedToolbarLog.Error("resolveOverlays exception", ex);
            }
        }
